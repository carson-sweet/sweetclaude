...
|---|---|---|
| **Go** | `/sweetclaude:go` | Pick up where you left off. Reads state, reads backlog from the SQLite cache, checks phase exit criteria, routes to the right skill. Pass plain-English arguments to describe what you want. |
...
| **Project Issues** | `/sweetclaude:project-issues` | Create, view, update, and close issues (`ISSUE-NNN`). Reads and writes issue files under `.sweetclaude/product/backlog/`. Closing an issue moves the file to `done/` and sets `closed_date`. Supports `list`, `backlog`, `view`, `new`, `update`, `close`, `reopen`. |
...
| **Project Backlog** | `/sweetclaude:project-backlog` | Backlog view grouped by priority bucket (NOW/SOON/LATER/SOMEDAY/UNESTIMATED). Reads from the SQLite cache. Promote issues to a sprint. Review imported issues. |
| **Project Backlog Triage** | `/sweetclaude:project-backlog-triage` | Structured grooming session. Sources issue files from `.sweetclaude/product/backlog/`. Works through ungroomed issues one at a time using INVEST criteria. Writes back to individual issue files. Sets status to `ready` when groomed. |
...
| **GitHub Import Issues** | `/sweetclaude:project-gh-import-issues` | Pull open GitHub Issues into `.sweetclaude/product/backlog/` as `ISSUE-NNN` files with `origin: imported`. Idempotent — issues already imported by GitHub number are skipped. Maps size/effort labels to effort field. |
| **GitHub Sync Issues** | `/sweetclaude:project-gh-sync-issues` | Bidirectional status sync against `.sweetclaude/product/backlog/` issue files only (roadmap sync is Phase 2). Pass 1: GH closed → local done (moves file to `done/`). Pass 2: local done → `gh issue close`. Reports counts for each direction. |
...
| **John Wick** | `/sweetclaude:john-wick` | Canonical 4.x entrypoint for complete large/high-rigor story workflows. Runs the autonomous multi-session SDLC pipeline with frozen binary success criteria, phase check-ins, and completion evidence gates. |
