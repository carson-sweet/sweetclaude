...
The test-writer/implementer pair is one of three named subagent patterns in the framework. The others are the caucus (parallel isolated agents that synthesize findings) and the John Wick escalation pattern (a privileged agent for enforcement tasks). Public docs describe them here at the conceptual level; the deeper architecture notes are internal maintainer material.
