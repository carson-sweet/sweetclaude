...
| **Update** | `/sweetclaude:update` | Sync stable-channel framework files inside a SweetClaude project after the Claude Code plugin package has been updated with `/plugin update sweetclaude@sweetclaude-stable`. Shows what changed and migrates supported 3.x project state when needed. |
...
| **Project Backlog** | `/sweetclaude:project-backlog` | Backlog view grouped by priority bucket (NEXT/SOONER/SOON/LATER/SOMEDAY/UNESTIMATED). Promote issues to a sprint. Surface inferred issues from Flow mode inference. |
