...
**Goal:** Remove [removed] wrapper infrastructure — delete absorbed skills, rename misnamed skills, update all framework files to remove [removed] dependencies.
...
### Task 4: Update master/SKILL.md — remove [removed] references

**Files:**
- Modify: `skills/master/SKILL.md`

Three [removed] references to replace (lines ~158–162):

Current text:
```
**For `[removed]`:** Conduct the full 11-section interview. One section at a time — never batch. Probe vague answers with follow-ups before moving to the next section. The interview is a discovery conversation, not a form to fill. After generating the document, run the [removed] validation checklist and present results before the phase gate.

**For `[removed]`:** Run all selected techniques to completion. Do not abbreviate a technique because you "have enough." The brainstorm output should contain quantified results (idea count, category count, insight count).

**For `[removed]`:** Answer every research question with evidence and sources. Identify research gaps explicitly. Do not present a research report with unanswered questions unless those gaps are flagged as open items.
```
...
```bash
grep -n "[removed]:" /Users/carsonsweet/dev/sweetclaude/skills/master/SKILL.md
```

- [ ] **Step 2: Edit the file** — replace the three `[removed]:` blocks with the new text above using the Edit tool. Replace each block individually to avoid mismatches.

- [ ] **Step 3: Verify no [removed] references remain**

```bash
grep -i "[removed]" /Users/carsonsweet/dev/sweetclaude/skills/master/SKILL.md
```
...
git commit -m "fix: remove [removed] references from master skill, replace with native skill guidance"
```

---

### Task 5: Update phase-gates.md — remove [removed] checklist reference
...
```
- [removed] validation checklist: the 9-item checklist has been run with all items passing or explicitly waived by the user with documented rationale
```
...
```bash
grep -i "[removed]" /Users/carsonsweet/dev/sweetclaude/rules/phase-gates.md
```
...
git commit -m "fix: replace [removed] validation checklist reference with native deliverable review criterion"
```

---

### Task 6: Update interaction-model.md — remove [removed] reference
...
```
Strip or ignore duration estimates from upstream workflows ([removed], Superpowers). Do not pass through time guidance when delegating to other frameworks.
```
...
```bash
grep -i "[removed]" /Users/carsonsweet/dev/sweetclaude/rules/interaction-model.md
```
...
git commit -m "fix: remove [removed] reference from interaction-model"
```
...
- [ ] **Step 2: Confirm no [removed] references remain in framework files**
