...
**Author:** Carson Sweet
**Status:** Draft
**Revised:** 2026-05-12 — v3.68.0: hook-based dispatch rebuild. Dispatch logic moved from inline SKILL.md bash into deterministic hook scripts (drift-gate.sh, master-preflight.sh). Versionless install path added (~/.claude/scripts/sweetclaude/). Migration helpers extracted from SKILL.md heredocs into standalone scripts. See Components 4, 5, 6, 14, and 14b (new).
**Revised:** 2026-05-01 — adds skills state management (Component 20): skills.yaml v2 schema, three-state model, pause operation, dependency registry, two-tier onboarding, offboard export verification, status read-only check. See also: phase/workflow separation (schema v2), native skills redesign, Protocol Guardian, John Wick mode
...
These eleven constraints shape every design decision:
...
| 11 | Subagent isolation | Information that flows backward from outcome to design corrupts the design | Caucus subagents cannot see each other's outputs; test-writer and implementer have separate context windows; git checkpoint enforces the barrier |
...
SweetClaude uses three named subagent patterns. Each has specific isolation rules and communication constraints.

### Caucus

N parallel isolated subagents — each assigned a distinct role or perspective — return structured findings independently. The orchestrating skill synthesizes them into a single output.

**Non-negotiable rule:** Subagents in a caucus cannot see one another's reasoning, drafts, or outputs. Cross-contamination collapses the diversity that makes the pattern work.

Each caucus instance defines:
- Role assignments (specialist roles, persona archetypes, stakeholder perspectives)
- A structured response schema each subagent must produce
- A synthesis stage that aggregates findings
- Mandatory role diversity — a caucus of identical roles is not a caucus

| Caucus | Roles | Invoked from | Status |
|---|---|---|---|
| **QA caucus** | Service/API, component/UI, integration | `code-feature` (TDD Level 3) | Implemented |
| **User focus group** | N synthetic persona archetypes from `state/personas.yaml` | `product-user-focus-group` | Implemented |
| **Stakeholder review** | CEO, CFO, CTO, CISO — pressure-testing a strategic decision | (future, not scoped) | Hypothetical |
| **Adversarial review** | Security, legal, compliance, accessibility — reviewing a design | (future, not scoped) | Hypothetical |

### Sequential Isolation (Test-Writer / Implementer)

Two subagents run in sequence with a deliberate information barrier between them. The test-writer sees Gherkin specs and writes failing tests; the implementer sees only the test files and makes them pass. Neither sees the other's reasoning. The barrier is enforced by git checkpoint (tests committed before implementation begins) and the test-guardian hook (test files immutable during implementation).

This pattern exists because test quality degrades when the test author knows the planned implementation — they write tests that confirm the approach rather than specify the behavior independently.

### Escalation (John Wick)

A single privileged subagent invoked when normal-agent behavior is insufficient — typically for security enforcement or protocol guardian tasks. Unlike caucus or sequential-isolation subagents, the John Wick agent operates with elevated authority rather than isolated perspective.
...
├── hooks/
│   └── sweetclaude/
│       ├── drift-gate.sh             # SessionStart — scans artifact-format drift; writes pending-drift-decision.yaml; emits AskUserQuestion JSON (v3.68.0)
│       ├── master-preflight.sh       # PreToolUse/Skill — blocks sweetclaude:* until bootstrap ran this session; lifecycle skills exempt (v3.68.0)
│       ├── session-preflight.sh      # SessionStart — health check, hook_last_ran timestamp
│       ├── test-guardian.sh          # PreToolUse — blocks test file edits during impl; enforces john-wick.yaml locked_test_files
...
├── scripts/
│   └── sweetclaude/
│       ├── preflight.sh              # Shared session pre-flight logic (sourced by skills)
│       ├── migrations/
│       │   ├── runner.py             # Registry-driven migration runner
│       │   ├── run_snapshot.py       # Pre-migration snapshot (idempotent via pending-migration-snapshot.json)
│       │   ├── run_migrate.py        # Executes migration run
│       │   └── run_rollback.py       # Executes rollback; clears snapshot marker on success
│       └── maintenance/
│           ├── ensure-global-hooks.py  # Registers required global hooks in settings.json on upgrade
│           ├── write-decline.py        # Writes framework.update.declined to sweetclaude.yaml
│           └── clear-decline.py        # Clears framework.update.declined (used by /sweetclaude:update)
│
└── config/
    └── sweetclaude/
        ├── defaults.yaml             # Default model routing, deference level, etc.
        ├── phase-skills.yaml         # Which skills/agents are available per phase
        └── skills-registry.yaml      # Static dependency graph for data-owning skills
```
...
│   ├── skills.yaml                   # Schema v2: onboarding state for 6 data-owning skills
...
**Implementation:** Shell script at `~/.claude/hooks/sweetclaude/test-guardian.sh` (registered as PreToolUse in `settings.json`)
...
**Implementation:** Shell script at `~/.claude/hooks/sweetclaude/auto-test-runner.sh` (registered as PostToolUse in `settings.json`)
**Loads:** Always active during Implement phase
...
**Implementation:** Shell script at `~/.claude/hooks/sweetclaude/git-checkpoint.sh` (registered as PostToolUse in `settings.json`)
...
- Read/write `state/skills.yaml` (via Component 20)
...
### Component 14: Drift Gate Hook _(v3.68.0)_
**Purpose:** SessionStart hook that detects artifact-format drift before any skill runs. Replaces bootstrap Step 5b's inline bash drift scan — dispatch moved out of SKILL.md into a deterministic hook.
**Responsibilities:**
- SessionStart: run the registry-driven migration runner in scan-only mode
- Write `.sweetclaude/state/pending-drift-decision.yaml` when drift is found
- Re-surface pre-existing marker without re-running the runner
- Emit AskUserQuestion JSON in `hookSpecificOutput.additionalContext` so Claude presents the migration prompt
- Silent for non-SweetClaude projects (no `.sweetclaude/state/sweetclaude.yaml`)
**Implementation:** Shell script at `~/.claude/hooks/sweetclaude/drift-gate.sh` (registered as SessionStart in `settings.json` by `ensure-global-hooks.py`)
**Loads:** Always active (global SessionStart hook)

### Component 14b: Master Pre-Flight Hook _(v3.68.0)_
**Purpose:** PreToolUse/Skill hook that enforces "bootstrap must run before any sweetclaude:* skill." Prevents /sweetclaude:go-style bypasses that skip bootstrap entirely.
**Responsibilities:**
- PreToolUse (matcher: Skill): intercept all skill invocations
- Pass non-sweetclaude:* skills through unconditionally
- Exempt lifecycle skills unconditionally: bootstrap, update, _migrate, purge, setup, adopt
- For all other sweetclaude:* skills: check for bootstrap session flag at `/tmp/.sweetclaude-bootstrap-ran-<project-hash>`; block with `{"ok": false}` if missing
- Session dedup via `/tmp` flag keyed to `md5(git rev-parse --show-toplevel)`
**Implementation:** Shell script at `~/.claude/hooks/sweetclaude/master-preflight.sh` (registered as PreToolUse in `settings.json` by `ensure-global-hooks.py`)
...
- Read `state/skills.yaml` (read-only — never writes or bootstraps it)
- Present consolidated status view
- Surface `SKILLS_WARNINGS` when a skill is `active` but its artifact is missing on disk
- Direct to `/sweetclaude:fix-sweetclaude` for any skills inconsistency — does not auto-correct
...
### Component 20: Skills State Manager
**Purpose:** Tracks onboarding state for the six data-owning skills — the skills that create and own data artifacts on disk: `product-backlog`, `product-milestones`, `product-sprint-plan`, `product-user-personas`, `product-user-stories`, `document-corpus`.
**Responsibilities:**
- Maintain `state/skills.yaml` (schema v2): `status` (`active | paused | uninitialized`) + `last_changed_at` + `last_changed_by` per skill
- **Dependency enforcement:** Each data-owning skill reads `config/skills-registry.yaml` at entry to resolve its dependency list; skills with unsatisfied dependencies halt with a clear message before doing any work
- **Three-state gate:** Each skill checks its own `status` at entry point:
  - `active` → proceed immediately
  - `paused` → "Resume? [yes/no]"
  - `uninitialized` on normal invocation → lightweight first-invocation flow (2 questions max, then proceed)
  - `$ARGUMENTS is onboard` → full onboarding ceremony regardless of current status
- **Two-tier onboarding:** First-invocation (triggered by `uninitialized` status) is lightweight — 2 questions, then continue to the originally requested operation. Explicit `onboard` argument triggers the full ceremony with structured interviews and broader context.
- **Pause operation:** Writing `status: paused` without deleting data files. Safe deactivation — resume by re-invoking normally. Different from offboard (data-destructive).
- **Offboard export verification:** Before showing the deletion confirmation, the offboard flow counts exported items (GitHub issues created, markdown files written, CSV rows) and compares against source count. Mismatch blocks deletion with "Continue anyway?" Choosing `none` as export method requires typing "NO BACKUP" explicitly.
- **Atomic write protocol:** All `skills.yaml` writes go through a temp file (`state/.skills.yaml.tmp`) then rename — prevents partial-write corruption from context window death.
- **Merge-not-clobber semantics:** Every write reads the current file, updates only the relevant skill entry, then writes back — never overwrites other skills' state.
- **Bootstrap and migration:** `update` skill handles v1→v2 migration (`enabled: true` → `status: active`, `enabled: false` with data → `status: paused`, `enabled: false` without data → `status: uninitialized`). `fix-sweetclaude` can infer state from artifact files and bootstrap a missing `skills.yaml`.
**Implementation:** Logic distributed across individual skill SKILL.md files; dependency registry at `config/skills-registry.yaml`; state file at `.sweetclaude/state/skills.yaml`
**Loads:** At entry point of each data-owning skill invocation

### Components 21-52: Domain Skills

All 52 skills are fully built and shipped. Each skill follows the standard SKILL.md format with a flat prefix naming convention. Domain grouping is by prefix, not directory hierarchy.
...
**Solution:** Single install mechanism — copy/clone SweetClaude files into `~/.claude/`. No compilation. Prerequisites: Claude Code CLI, git, GitHub CLI (`gh`), Superpowers plugin. The installer validates all prerequisites before proceeding. [removed] is no longer required — all SweetClaude skills are native.
...
| Defaults | Config |
| Skills Registry | Config (`config/skills-registry.yaml`) |
...
*Originally generated 2026-04-13. Revised 2026-04-29 to reflect phase/workflow separation (schema v2), native skills redesign, Protocol Guardian, and John Wick mode. Revised 2026-05-01 to add Component 20 (Skills State Manager): skills.yaml v2 schema, three-state lifecycle model, pause operation, dependency registry, two-tier onboarding, offboard export verification, status read-only invariant.*
