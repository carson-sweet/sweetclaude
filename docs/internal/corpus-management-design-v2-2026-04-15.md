...
**Version:** 2.0
**Status:** Superseded by implementation — see implementation note below
**Authors:** Carson Sweet, Claude (Claude Code instance)
**Supersedes:** corpus-management-design-v1-2026-04-14.md

---

## ⚠ Implementation Note (added 2026-05-05)

This design document describes six standalone skills (`corpus-consolidate`, `corpus-triage`, `corpus-reconcile`, `corpus-promote`, `corpus-reindex`, `corpus-status`) as separate SKILL.md files. **None of those files exist.** All six were consolidated into a single skill with `$ARGUMENTS`-based mode routing:

```
skills/document-corpus/SKILL.md
```

Invoked as `/sweetclaude:document-corpus` with an optional mode argument (e.g., `/sweetclaude:document-corpus triage`). The skill presents a mode menu when invoked with no argument.

**Mode mapping:**

| Design doc skill | Actual implementation |
|---|---|
| `corpus-consolidate` | `## Mode: Consolidate` in `document-corpus` (line 393) |
| `corpus-triage` | `## Mode: Triage` in `document-corpus` (line 570) |
| `corpus-reconcile` | `## Mode: Reconcile` in `document-corpus` (line 664) |
| `corpus-promote` | `## Mode: Promote` in `document-corpus` (line 784) |
| `corpus-reindex` | `## Mode: Reindex RAG` in `document-corpus` (line 1043) |
| `corpus-status` | `## Mode: Status` in `document-corpus` (line 321) |

The pipeline state machine (`corpus-pipeline.yaml`), gating rules, and directory structure described below remain accurate. The skill interface is different — there is one skill, not six.

Do not propose building separate `corpus-consolidate/SKILL.md`, `corpus-triage/SKILL.md`, etc. — they already exist as modes inside `document-corpus`.
...
### 1. Consolidate (`/sweetclaude:document-corpus consolidate`)

**Status:** Implemented as `## Mode: Consolidate` in `skills/document-corpus/SKILL.md` (line 393)
...
### 2. Triage (`/sweetclaude:document-corpus triage`)

**Status:** Implemented as `## Mode: Triage` in `skills/document-corpus/SKILL.md` (line 570)
...
### 3. Reconcile (`/sweetclaude:document-corpus reconcile`)

**Status:** Implemented as `## Mode: Reconcile` in `skills/document-corpus/SKILL.md` (line 664)
...
### 4. Promote (`/sweetclaude:document-corpus promote`)

**Status:** Implemented as `## Mode: Promote` in `skills/document-corpus/SKILL.md` (line 784)
...
### 5. Reindex (`/sweetclaude:document-corpus reindex`)

**Status:** Implemented as `## Mode: Reindex RAG` in `skills/document-corpus/SKILL.md` (line 1043)
...
4. **If RAG tooling is not available:**
   > "MCP RAG server not configured. Run `/sweetclaude:document-corpus` and select **Set up RAG** first."

### 6. Corpus Status (`/sweetclaude:document-corpus status`)

**Status:** Implemented as `## Mode: Status` in `skills/document-corpus/SKILL.md` (line 321)
...
4. If `pipeline.interrupted` or `pipeline.step != idle`:
   > "Warning: {step} did not complete. Run `/sweetclaude:document-corpus {mode}` to resume or `/sweetclaude:document-corpus status` for details."
...
**All items complete.** See implementation note at the top of this document.

All six modes were consolidated into `skills/document-corpus/SKILL.md` rather than built as separate skill files:

1. ~~Update corpus-consolidate to write `corpus-pipeline.yaml` on entry/exit~~ — implemented as `## Mode: Consolidate`
2. ~~Update corpus-triage to check gate and write pipeline state on entry/exit~~ — implemented as `## Mode: Triage`
3. ~~Build corpus-reconcile~~ — implemented as `## Mode: Reconcile`
4. ~~Build corpus-promote~~ — implemented as `## Mode: Promote`
5. ~~Build corpus-reindex~~ — implemented as `## Mode: Reindex RAG`
6. ~~Build corpus-status~~ — implemented as `## Mode: Status`
7. ~~Add pipeline state check to corpus preflight hook~~ — gate checks are inline in each mode's Step 0
