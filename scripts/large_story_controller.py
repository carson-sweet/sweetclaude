    ...
    if resolved_workflow_id and _valid_workflow_id(resolved_workflow_id):
        ...
        if wf_state.get("status") == "complete":
            ...
            return {
                ...
                "message": "Large-story already complete; cleared stale active work item.",
            }
...
def _check_init_preconditions(
    ...
    if not current_branch or (current_branch != main_branch and current_branch != story_branch):
        ...
        return _failure(
            "blocked_not_on_main",
            f"Large-story init is blocked: current branch is '{display}', "
    ...
    try:
        ...
        if r.returncode == 0 and _blocking_dirty_entries(r.stdout):
            return _failure(
                "blocked_dirty_tree",
                "Large-story init is blocked: working tree has uncommitted changes. "
                "Commit, stash, or discard changes before starting a new story.",
            )
        if r.returncode != 0:
            return _failure(
                "blocked_dirty_tree",
                "Large-story init is blocked: git status failed. "
        ...
        for candidate in sorted(workflows_dir.glob("*.yaml")):
            ...
            if (
                ...
                return _failure(
                    ...
                    f"Large-story init is blocked: active workflow {wf_id} ({owner}) "
    ...
    if find_backlog_file(project, workflow_id, exclude_done=True) is None:
        return {
            **_failure(
                ...
                f"Large-story init is paused: no backlog file exists for {workflow_id} yet. "
        ...
        if (
            ...
            if rel == PROTECTED_PHASE_REL:
                return _decision(
                    ...
                    "run `python3 scripts/large_story_controller.py ship "
            ...
            if token in lowered:
                if "phase.yaml" in token:
                    return _decision(
                        ...
                        "work item, run `python3 scripts/large_story_controller.py "
...
def _is_active_workflow_state(state: dict[str, Any]) -> bool:
    return bool(
        state
        and state.get("state_owner") == "large_story_controller"
        and state.get("requires_success_criteria_contract")
        and state.get("status") != "complete"
    )
...
def _completed_workflow_protected_paths(
    ...
    protected_dir_prefixes: directory prefixes whose entire subtree is
        immutable (per-story report dirs like reports/large-story/ISSUE-047/).
    """
