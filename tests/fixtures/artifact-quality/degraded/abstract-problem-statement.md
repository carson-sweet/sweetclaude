...
violates: BRIEF-CONCRETE-SCENARIO
note: "Problem statement is a category, not a specific scenario."
---
...
Developers sometimes have trouble when things go wrong with their tooling. This is a common problem in the industry and causes frustration.
...
1. Running `/sweetclaude:doctor` on a project with all 10 known problem types surfaces every one of them in a single report — zero require a separate skill invocation to discover.
2. Auto-fixable findings (counter drift, version reconciliation, stale markers, plan directory, session-state regen) are fixed without prompting and reported as fixed.
3. No finding from `fix-sweetclaude`, `_health`, `hook-repair`, `migrate-diagnose`, `claude-config-audit`, `drift-gate.sh`, `sweetclaude-health-check.sh`, `emergency-hook-restore.sh`, update Step 6b2, or `scan-orphans` is missing from doctor's coverage.
4. The bootstrap prompt fires after 7+ days and does not fire before.
5. The backing script is testable independently of the skill (same pattern as `migrate-v3-to-v4.py`).
